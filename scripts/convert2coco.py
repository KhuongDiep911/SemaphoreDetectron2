"""
    ...
    > python convert2coco.py  meta_file_path(~/meta.json)  annotation_directory_path(~/ann)  save_directory_path(~/,default = ./result.json)
...
def convert_supervisely_to_coco(meta_path, ann_dir, save_dir='./'):
    ...
    coco_fmt = {
        ...
        "licenses": [
            {
                "id": 10421394, # set the license id
                "name": "keypoint"  # set the license name
            }
        ],
        "images": images_repr,
...
def divide_and_generate_annots(idimg, fname_img, json_suprv, categories, imgs_base_dir, node_names, start_annot_id=10000):
    ...
    image_annot = {
            "license": 10421394, # set the id of license ex)"licenses": 1
            "file_name": fname_img,
...
def main():
    ...
    Converting from suvervisely output to COCO data format
    Example of Usage in commandline:
        > python convert2coco.py  meta_file_path('~/meta.json')  annotation_directory_path('~/ann')  save_path(~/)
    """)
    parser.add_argument(
        "-v",
        "--version",
